...
$(document).ready(async function () {
    ...
    async function getDetails() {
        var id = $(this).data("id");
        var selectedData = await getData(`/Deduction/${id}`);
    ...
    $("#btnAdd").click(async function () {
        ...
        myModal.hide();
        await postData("/Deduction", dataInsert);
        var listData = await getList("/Deduction");
    ...
    $("#btnSave").click(async function () {
        ...
        myModal.hide();
        await putData(`/Deduction/${updatedData.id}`, updatedData);
        var listData = await getList("/Deduction");
        newGrid.updateData(listData)
    });
    ...
    $("#delete-notification").click(async function () {
        ...
        await putStatus(`/Deduction/DeletedStatus/${id}/${true}`);
        var listData = await getList("/Deduction");
        newGrid.updateData(listData)
    });
    ...
    $("#btnFilter").click(async function () {
        ...
        var listData = await getList(`/Deduction/Search/${field}/${keyword}`);
        console.log(listData)
        newGrid.updateData(listData)
    ...
    function getDataForm() {
        //console.log(pond)
        return {
            id: $("#deduction_id").val() == "" ? 0 : $("#deduction_id").val(),
            name: $("#name").val(),
            amount: $("#amount").val(),
            note: $("#note").val(),
            isDeleted: false,
