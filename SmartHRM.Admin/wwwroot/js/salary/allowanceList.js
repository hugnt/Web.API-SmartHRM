...
$(document).ready(async function () {
    var listData = await getList("/Allowance");
    ...
    async function getDetails() {
        var id = $(this).data("id");
        var selectedData = await getData(`/Allowance/${id}`);
    ...
    $("#btnAdd").click(async function () {
        ...
        myModal.hide();
        await postData("/Allowance", dataInsert);
        var listData = await getList("/Allowance");
    ...
    $("#btnSave").click(async function () {
        ...
        myModal.hide();
        await putData(`/Allowance/${updatedData.id}`, updatedData);
        var listData = await getList("/Allowance");
        newGrid.updateData(listData)
    });
    ...
    $("#delete-notification").click(async function () {
        ...
        await putStatus(`/Allowance/DeletedStatus/${id}/${true}`);
        var listData = await getList("/Allowance");
        newGrid.updateData(listData)
    });
    ...
    $("#btnFilter").click(async function () {
        ...
        var listData = await getList(`/Allowance/Search/${field}/${keyword}`);
        console.log(listData)
        newGrid.updateData(listData)
    ...
    function getDataForm() {
        //console.log(pond)
        return {
            id: $("#bonus_id").val() == "" ? 0 : $("#bonus_id").val(),
            name: $("#name").val(),
            amount: $("#amount").val(),
            note: $("#note").val(),
            isDeleted: false,
    ...
    async function isValidForm(data) {
        ...
        if (data.name == "" || data.name == null) {
            message = "Allowance name is required!";
        }
        else if (data.amount == null || data.amount == "") {
            message = "Allowance amount is required!";
        }
        else if (data.amount < 0) {
            message = "Allowance amount is not valid!";
        }
        if (message != null) {
            Toastify({
                text: message,
